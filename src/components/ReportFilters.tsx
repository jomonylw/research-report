...
const VirtualizedList = ({
  ...
  return (
    <Command>
      ...
      <CommandList
        ref={parentRef}
        style={{ height: '224px', overflow: 'auto' }}
      >
...
export const ReportFilters = ({ onFilterChange }: ReportFiltersProps) => {
  ...
  const [filters, setFilters] = useState<FilterState>({
    ...
    industryCode:
      searchParams.get('industryCode')?.split(',').filter(Boolean) || [],
    columnCode:
      searchParams.get('columnCode')?.split(',').filter(Boolean) || [],
  ...
  const updateQuery = (
    ...
    if (
      ...
      (typeof value === 'number' && (key !== 'attachPages' || value !== 0))
    ) {
      ...
      <div className='p-4 border rounded-lg bg-card mb-4'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
            ...
            >
              <Command>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    <CommandItem
                      onSelect={() => handleIndustryChange('all', true)}
                    >
                      <div
                        className={cn(
                          'mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary',
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant='outline' className='w-full justify-start'>
                <span>
                  {filters.stockCode
                    ? stockList.find((s) => s.value === filters.stockCode)
                        ?.label
                    : '选择股票 ...'}
                </span>
              </Button>
          ...
          <div className='lg:col-span-4 flex items-center space-x-2'>
            <Button
              variant={!filters.reportType ? 'secondary' : 'outline'}
              onClick={() => handleReportTypeClick('')}
            >
              全部类型
            </Button>
            <Button
              variant={filters.reportType === '2' ? 'secondary' : 'outline'}
              onClick={() => handleReportTypeClick('2')}
            >
              个股研报
            </Button>
            <Button
              variant={filters.reportType === '3' ? 'secondary' : 'outline'}
              onClick={() => handleReportTypeClick('3')}
            >
              行业研报
            </Button>
          </div>
