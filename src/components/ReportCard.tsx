...
interface ReportCardProps {
  report: Report;
  onTagClick: (filterType: 'orgCode' | 'author' | 'industryCode' | 'stockCode' | 'reportType' | 'columnCode', value: string) => void;
}
...
export const ReportCard = ({ report, onTagClick }: ReportCardProps) => {
  ...
  const highlightKeywords = (htmlContent: string, query: string | null): string => {
  ...
  const handleTagClick = (e: React.MouseEvent, filterType: 'orgCode' | 'author' | 'industryCode' | 'stockCode' | 'reportType' | 'columnCode', value: string | null) => {
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <p className="text-sm text-muted-foreground whitespace-nowrap">
          {report.publishDate ? new Date(report.publishDate).toLocaleDateString() : 'N/A'}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-2 mt-2 items-center">
        ...
        {report.stockCode && report.stockName && (
          <button onClick={(e) => handleTagClick(e, 'stockCode', report.stockCode)} className="text-sm bg-blue-100 text-blue-800 hover:bg-blue-200 px-2 py-1 rounded-md dark:bg-blue-900 dark:text-blue-300">
            {report.stockName} ({report.stockCode})
          </button>
        )}
        {report.column && columnLabel && (
          <button onClick={(e) => handleTagClick(e, 'columnCode', report.column)} className="text-sm bg-purple-100 text-purple-800 hover:bg-purple-200 px-2 py-1 rounded-md dark:bg-purple-900 dark:text-purple-300">
            {columnLabel}
          </button>
        )}
        {industryDisplayCode && industryDisplayName && (
          <button onClick={(e) => handleTagClick(e, 'industryCode', industryDisplayCode)} className="text-sm bg-muted hover:bg-muted/80 px-2 py-1 rounded-md">
            {industryDisplayName}
          </button>
        )}
        {report.orgCode && report.orgSName && (
          <button onClick={(e) => handleTagClick(e, 'orgCode', report.orgCode)} className="text-sm bg-muted hover:bg-muted/80 px-2 py-1 rounded-md">
            {report.orgSName}
          </button>
        )}
        {report.authorNames.length > 1 && !authorsExpanded ? (
          <>
            <button onClick={(e) => handleTagClick(e, 'author', report.authors[0])} className="text-sm bg-muted hover:bg-muted/80 px-2 py-1 rounded-md">
              {report.authorNames[0]}
            </button>
            <button onClick={(e) => { e.stopPropagation(); setAuthorsExpanded(true); }} className="text-sm bg-muted hover:bg-muted/80 px-2 py-1 rounded-md">
              ...
            </button>
          </>
        ) : (
          report.authorNames.map((name, index) => (
            <button key={index} onClick={(e) => handleTagClick(e, 'author', report.authors[index])} className="text-sm bg-muted hover:bg-muted/80 px-2 py-1 rounded-md">
              {name}
            </button>
          ))
        )}
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-0' : 'max-h-[200px] mt-3'}`}>
        <p>{report.summary}</p>
      </div>
      ...
      >
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: highlightKeywords(report.content || '', contentQuery) }}
        />
      </div>
        ...
        {report.pdfLink && (
          <Button asChild size="sm" variant="outline" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              阅读 {report.attachSize ? `(${(report.attachSize / 1024 ).toFixed(2)} MB)` : ''}
            </a>
          </Button>
        )}
      </div>
